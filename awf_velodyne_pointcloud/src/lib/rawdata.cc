...
/** @brief convert raw packet to point cloud
  ...
  void RawData::unpack(const velodyne_msgs::msg::VelodynePacket & pkt, velodyne_pointcloud::OutputBuilder & data, size_t idx, size_t offset)
  {
//    RCLCPP_INFO_STREAM(node_ptr_->get_logger(), "Unpacking packet: " << idx << " offset: " << offset);
    using velodyne_pointcloud::LaserCorrection;
    RCLCPP_DEBUG_STREAM(
      node_ptr_->get_logger(), "Received packet, time: " << rclcpp::Time(
                                                              pkt.stamp).seconds());
    ...
    /** special parsing for the VLP16 **/
    if (calibration_.num_lasers == 16) {
      unpack_vlp16(pkt, data, idx, offset);
      return;
    }

    /** special parsing for the VLS128 **/
    if (calibration_.num_lasers == 128) {
      unpack_vls128(pkt, data, idx, offset);
      return;
    }
                                                     ...
                                                     raw->blocks[i].rotation >= config_.min_angle)))
        {
          ...
          if (is_invalid_distance) {
            data.addPointWithIndex(
              x_coord, y_coord, z_coord, return_type, corrections.laser_ring,
              raw->blocks[i].rotation, 0,
              intensity, time_stamp, idx, scansPerPacket(), offset, index);
          } else {
            data.addPointWithIndex(
              x_coord, y_coord, z_coord, return_type, corrections.laser_ring,
              raw->blocks[i].rotation, distance,
...
//                RCLCPP_INFO_STREAM(node_ptr_->get_logger(), "Will add point");
//                RCLCPP_INFO_STREAM(node_ptr_->get_logger(), "Packet id: " << idx << " offset: " << offset << "Point id: " << index);
                if (is_invalid_distance) {
                  data.addPointWithIndex(
                    x_coord, y_coord, z_coord, return_type, corrections.laser_ring,
                    azimuth_corrected, 0, intensity, time_stamp, idx, scansPerPacket(), offset, index);
                } else {
                  data.addPointWithIndex(
                    x_coord, y_coord, z_coord, return_type, corrections.laser_ring,
                    azimuth_corrected, distance, intensity, time_stamp, idx, scansPerPacket(), offset, index);
                }
                ++index;
              }
            }
